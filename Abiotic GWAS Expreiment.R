...
jin2=read.csv( "growdata_mic0.csv",header = T,row.names = 1)#Growth data of Staphylococcus aureus in mic=0
jin3=read.csv( "growdata_mic2.csv",header = T,row.names = 1)#Growth data of Staphylococcus aureus in mic=2
jin4=read.csv( "growdata_mic4.csv",header = T,row.names = 1)#Growth data of Staphylococcus aureus in mic=4
jin5=read.csv( "growdata_mic6.csv",header = T,row.names = 1)#Growth data of Staphylococcus aureus in mic=5
SNP=read.csv( "snpdata.csv",header = T,row.names = 1)#snp data
k8str <- read.csv("FASTSTRUCTURE.csv", header = F)#faststructure results
...
get_cl_meancurve <- function(data,clust){
  ...
  fc_res[[1]] <- marker_list
  
  fc_res[[2]] <- F2014_H.odee
  names(fc_res)<-c("lasso_res","ode_res"
  )
  return(fc_res) 
