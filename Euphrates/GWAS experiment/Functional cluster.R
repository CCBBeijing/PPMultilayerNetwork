...
ykmeans <- function(data,k,iter.max=10){
  ...
  smdist <- order(mdist,decreasing = T)[1:(k+20)]
  
  ssmdist <- dist(data[smdist,],method = "euclidean")
  DIS  <- as.matrix(ssmdist)
  diss <- c()
  for (i in 1:(k+20)) {
    
    diss <- c(diss,which(as.numeric(DIS[i,1:(i-1)]) < 0.01)) 
    
    
  }
  fale <- as.numeric(names(table(diss)))
  center <- data[smdist[-fale],]
  center <- center[1:k,]
...
input=get_init_par(df,66)
cluster_par <- get_cluster(df,66,input) 
