...
pheno_clusterall <-matrix(NA,ncol = length(t),nrow = 66)
for(e in 1:66){
...
rownames(pheno_clusterall) <- seq(1,66,1)
...
for (col in 1:length(marker[1,])) {
  ...
  ridge1_cv <- cv.glmnet(x = x_matrix, y = m,type.measure = "mse",nfold = 500,alpha = 0)
  best_ridge_coef <- as.numeric(coef(ridge1_cv, s = ridge1_cv$lambda.min))[-1]
  fit_res <- cv.glmnet(x = x_matrix, y = m,type.measure = "mse",nfold = 500,alpha = 1,penalty.factor = 1 / abs(best_ridge_coef),keep = TRUE)
  best_alasso_coef1 <- coef(fit_res, s = fit_res$lambda.min)
  marker_list_one <- list()
  marker_list_one[[1]] <- name[col]
  marker_list_one[[2]] <- best_alasso_coef1@Dimnames[[1]][best_alasso_coef1@i[-1]+1]
  marker_list_one[[3]] <- best_alasso_coef1@x[-1]
...
F2015_H.od <- optim.parallel(connect=lasso,effect=t(pheno_clusterall),
                             n.cores=1,proc=ode.optim,order=8,times=seq(20,120,length=26),nstep=25)
